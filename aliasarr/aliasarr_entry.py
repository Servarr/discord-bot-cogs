...
class AliasCache:
    def __init__(self, config: Config, cache_enabled: bool = True):
        ...
        self._loaded = False
        self._aliases: Dict[Optional[int], Dict[str, AliasarrEntry]] = {None: {}}
            ...
            for a in global_aliases:
                if a.get("creator", 0) == user_id:
                    ...
                    if self._cache_enabled:
                        self._aliases[None][a["name"]] = AliasarrEntry.from_json(a)
            ...
            async with self.config.guild_from_id(guild_id).entries() as entry_list:
                for a in entry_list:
                    if a.get("creator", 0) == user_id:
                        a["creator"] = 0xDE1
                        if self._cache_enabled:
                            self._aliases[guild_id][a["name"]] = AliasarrEntry.from_json(a)
    ...
    async def load_aliases(self):
        ...
        for alias in await self.config.entries():
            self._aliases[None][alias["name"]] = AliasarrEntry.from_json(alias)
        ...
        async for guild_id, guild_data in AsyncIter(all_guilds.items(), steps=100):
            ...
            for alias in guild_data["entries"]:
                self._aliases[guild_id][alias["name"]] = AliasarrEntry.from_json(alias)
        self._loaded = True

    async def get_aliases(self, ctx: commands.Context) -> List[AliasarrEntry]:
        """Returns all possible aliases with the given context"""
        global_aliases: List[AliasarrEntry] = []
        server_aliases: List[AliasarrEntry] = []
    ...
    async def get_guild_aliases(self, guild: discord.Guild) -> List[AliasarrEntry]:
        """Returns all guild specific aliases"""
        aliases: List[AliasarrEntry] = []
        ...
        if self._cache_enabled:
            ...
            aliases = [AliasarrEntry.from_json(d) for d in await self.config.guild(guild).entries()]
        return aliases

    async def get_global_aliases(self) -> List[AliasarrEntry]:
        ...
        if self._cache_enabled:
            ...
            aliases = [AliasarrEntry.from_json(d) for d in await self.config.entries()]
        return aliases

    async def get_alias(
        self, guild: Optional[discord.Guild], alias_name: str
    ) -> Optional[AliasarrEntry]:
        """Returns an AliasarrEntry object if the provided alias_name is a registered alias"""
        server_aliases: List[AliasarrEntry] = []
        ...
        if self._cache_enabled:
            ...
            if guild:
                server_aliases = [
                    AliasarrEntry.from_json(d) for d in await self.config.guild(guild.id).entries()
                ]
            global_aliases = [AliasarrEntry.from_json(d) for d in await self.config.entries()]
    ...
    async def add_alias(
        ...
        if global_:
            alias = AliasarrEntry(alias_name, command, ctx.author.id, None)
            settings = self.config
            if self._cache_enabled:
                self._aliases[None][alias.name] = alias
        else:
            alias = AliasarrEntry(alias_name, command, ctx.author.id, ctx.guild.id)
    ...
    async def edit_alias(
        ...
        async with settings.entries() as aliases:
            for index, alias in enumerate(aliases):
                if alias["name"] == alias_name:
                    alias_edited = AliasarrEntry.from_json(alias)
